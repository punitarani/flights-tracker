...
describe("seats-aero-cache-db", () => {
  describe("Search Request Management", () => {
    it("should create a search request", async () => {
    ...
    it("should upsert search request on duplicate route/dates", async () => {
      const params = {
        originAirport: "LAX",
    ...
    it("should retrieve an existing search request", async () => {
    ...
    it("should update search request progress", async () => {
      const request = await createSearchRequest({
        originAirport: "DFW",
    ...
    it("should mark search request as completed", async () => {
      const request = await createSearchRequest({
        originAirport: "SEA",
    ...
    it("should mark search request as failed", async () => {
      const request = await createSearchRequest({
        originAirport: "PHX",
  ...
  describe("Availability Trip Management", () => {
    it("should upsert an availability trip", async () => {
    ...
    it("should update existing trip on conflict", async () => {
      const searchRequest = await createSearchRequest({
        originAirport: "JFK",
    ...
    it("should get availability trips with filters", async () => {
      const searchRequest = await createSearchRequest({
        originAirport: "ORD",
      ...
      expect(economyTrips.every((t) => t.cabinClass === "economy")).toBe(true);
      ...
      expect(dateTrips.every((t) => t.travelDate === "2025-10-05")).toBe(true);
    });
  });

  describe("Cleanup Operations", () => {
    it("should delete expired records", async () => {
      // Create a search request with very short TTL
      const now = new Date();
      const _expiredAt = new Date(now.getTime() - 1000); // 1 second ago

      const _request = await createSearchRequest({
        originAirport: "EXP",
        destinationAirport: "IRE",
        searchStartDate: "2025-11-01",
        searchEndDate: "2025-11-10",
      ...
      const { searchRequests, trips } = await deleteExpiredRecords();

      expect(searchRequests).toBeGreaterThanOrEqual(0);
      expect(trips).toBeGreaterThanOrEqual(0);
    });

    it("should delete route records", async () => {
