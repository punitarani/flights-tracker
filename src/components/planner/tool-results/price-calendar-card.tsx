...
interface PriceCalendarCardProps {
  route: string;
  prices: PricePoint[];
...
/**
 * Enhanced Price Calendar Card with AI Elements integration
 * Visual bar chart showing price trends across dates with interactive actions
 */
export function PriceCalendarCard({
  route,
  prices,
  cheapestPrice,
  cheapestDate,
          ...
          {prices.slice(0, 10).map((entry) => {
            ...
            return (
              <div key={entry.date} className="flex items-center gap-2 text-sm">
                ...
                {onSearchDate && isLowest && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-7 text-xs"
                    onClick={() => onSearchDate(entry.date)}
                  >
                    Search
                  </Button>
                )}
